	...
	<script src="${APP_PATH}/layer/layer.js"></script>
        <script type="text/javascript">
            ...
            function pageQuery(pageIndex) {
            	...
            	$.ajax({
            		url : "${APP_PATH}/manager/auth/pageQuery",
            		type : "POST",
            		data : jsonData,
            		beforeSend : function() {
            		...
            		success : function(result) {
            			...
            			if(result.success) {
            				...
            				$.each(tasks, function(i, task){
            	                taskRows = taskRows +  '<tr>';
	          	                taskRows = taskRows +  '  <td>'+(i+1)+'</td>';
	          	                taskRows = taskRows +  '  <td>'+task.pdname+'</td>';
	          	                taskRows = taskRows +  '  <td>'+task.pdversion+'</td>';
	          	                taskRows = taskRows +  '  <td>'+task.taskname+'</td>';
	          	                taskRows = taskRows +  '  <td>'+task.membername+'</td>';
	          	                taskRows = taskRows +  '  <td>';
	          					taskRows = taskRows +  '      <button type="button" onclick="goAuthPage(\''+task.taskid+'\', \''+task.memberid+'\')" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
	          					taskRows = taskRows +  '  </td>';
	          	                taskRows = taskRows +  '</tr>';
            				});
            				
            				$("#taskBody").html(taskRows);
            ...
            function goAuthPage(taskid, memberid) {
                window.location.href = "${APP_PATH}/manager/auth/detail?taskid="+taskid+"&memberid="+memberid;
            }
