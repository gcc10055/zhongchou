...
.seltype {
 ...
 <div class="navbar-wrapper">
	...
	<button type="button" class="btn btn-danger btn-lg btn-block" onclick="next()">认证申请</button>
    </div> <!-- /container -->
      <!-- /END THE FEATURETTES -->
    ...
    $("img").each(function(){
        ...
        $(this).attr("src", "${APP_PATH}/" + src);
    });
    var accttype = null;
    function next() {
        if ( accttype == null ) {
        	layer.msg("请选择账户类型。", {time:2000, icon:5, shift:6}, function(){
            ...
            $.ajax({
                ...
                success : function(result) {
                    if ( result.success ) {
			        	...
			        	layer.msg("账户类型更新失败。", {time:2000, icon:5, shift:6}, function(){
